...
import com.zak.backup.utils.OSUtil;
...
public class LocalOracleBackup extends AbstractDatabaseBackup {
    ...
    protected String doBackup() {
        ...
        // 获取分隔符
        String separator = ":";
        if (backupConfig.getJdbcUrl().contains(String.format("%s:%d/%s", backupConfig.getHost(), backupConfig.getPort(), backupConfig.getDatabaseName()))
                || backupConfig.getJdbcUrl().contains(String.format("%s/%s", backupConfig.getHost(), backupConfig.getDatabaseName()))) {
            separator = "/";
        }
        ...
        if (cmdResult.getCode() != 0) {
            if (cmdResult.getError().contains("ORA-12541")) {
